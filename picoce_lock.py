...
def signal_handler(sig, frame, lock_file, logger=None):
    logger = get_logger(logger)
...
def acquire_lock(lock_file, duration_s=0, logger=None):
    logger = get_logger(logger)
    logger.info(f"Acquiring lock file: {lock_file}")
    ...
    if unlocked:
        try:
            ...
            logger.info("New lock acquired successfully.")
            return True
        except IOError as e:
            logger.error(f"Failed to acquire new lock: {e}")
            return False
    else:
        logger.info("Existing lock is still valid; new lock not acquired.")
        return False
        ...
        with open(lock_file, 'r+') as f:
            try:
                ...
                try:
                    ...
                    logger.info(f"Lock file content: {lock_content}")
                    ...
                    logger.info(f"Current time: {current_time}, Expiration time: {expiration_timestamp}")
                ...
                logger.info("Lock is currently held by another process.")
                return False
